...
export function IOSInstallGuideModal({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 grid place-items-center bg-[#0F4C5C]/40 px-4 py-4 backdrop-blur-sm">
      <div className="max-h-[calc(100dvh-32px)] w-full max-w-[400px] overflow-y-auto rounded-[24px] border border-slate-200 bg-white p-6 shadow-[0_24px_80px_rgba(17,17,17,.12)]">
        <div className="mb-4 flex items-center justify-between border-b border-slate-100 pb-3">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="font-display text-[16px] font-bold text-[#0F4C5C]">Install Fabric Care</h3>
              <p className="text-[10px] text-slate-500">Fast home screen access & offline mode</p>
            </div>
          </div>
          <button
            onClick={onClose}
            className="grid size-8 place-items-center rounded-lg bg-slate-100 text-slate-600 hover:bg-slate-200 hover:text-slate-800 transition"
            aria-label="Close"
          >
            <X className="size-4" />
          </button>
        ...
        <div className="space-y-3.5 text-xs text-slate-700">
          <div className="rounded-xl bg-slate-50 border border-slate-200/80 p-3.5 space-y-2">
            ...
            <ol className="list-decimal pl-4 space-y-2 text-[11px] leading-relaxed text-slate-600">
              <li className="pl-1">
                ...
                <Share2 className="inline size-3.5 text-[#0F4C5C]" /> in the browser bar.
              </li>
              <li className="pl-1">
                Scroll down the options and select{" "}
                <strong className="text-[#0F4C5C]">"Add to Home Screen"</strong>{" "}
                <PlusSquare className="inline size-3.5 text-[#0F4C5C]" />.
              </li>
              <li className="pl-1">
          ...
          <div className="flex items-center gap-2 text-[10px] text-slate-500 bg-emerald-50 border border-emerald-200/60 p-2.5 rounded-xl">
          ...
          <button
            onClick={onClose}
            className="w-full mt-2 py-2.5 bg-[#0F4C5C] text-white text-xs font-bold rounded-xl hover:bg-[#0F4C5C]/90 transition"
          >
            Got it
          </button>
        </div>
